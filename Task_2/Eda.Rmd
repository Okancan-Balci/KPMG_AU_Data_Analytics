...
## Cleaning the new data New Customer List
...
```{r}
Customer_Joined %>% 
  inner_join(response_table, by = "customer_id") 
```

Seems like some of the customers didn't buy anything in 2017. I think leaving them out would be wrong and we would lose a good chunk of our data. After I join these two tables I am also going to set profit values to zero for these consumers. This way Machine Learning algorithm can learn what kind of consumers don't make any transactions.
...
Positives:
(3550, 3750], (2850, 3050], (3350, 3450], (4750, 4850]
...
Positives:
Entertainment, Financial Services, IT

Negatives:
Argiculture(yeah it is a typo in the data)
...
Positives:
All the manager titles I should probably create separate dummies.
...
diverging_job_titles <- Customer_Joined %>% 
  ...
  mutate(median_ratio = median_profit/median(Customer_Joined$profit_per_transaction)) %>% 
  arrange(desc(median_ratio)) %>% 
  filter(median_ratio > 1.2 | median_ratio < 0.8) %>% 
  pull(job_title)
```
